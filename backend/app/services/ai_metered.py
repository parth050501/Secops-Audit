...
async def _check_and_consume(db: AsyncSession, tenant_id: int, user, operation: str,
    ...
    """Credits removed: AI features are no longer metered or gated. We still record
    a usage entry (useful for visibility/analytics), but never block a call."""
    db.add(AIUsage(
        tenant_id=tenant_id,
        user_id=user.id,
        user_name=user.name,
        ...
        entity_id=entity_id,
        credits_used=0,
        cost_usd=USD_COSTS.get(operation, 0.0),
    ))
    await db.commit()
