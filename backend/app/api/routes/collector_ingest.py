...
def _parse(system_type: str, raw: Any, tenant_id: int, ref_id: int, framework: Optional[str]):
    if system_type == "linux":
        ...
        from app.connectors.mysql_db import assess_mysql_config
        return assess_mysql_config(raw, tenant_id, ref_id, framework_hint=framework)
    elif system_type == "mssql":
        from app.connectors.mssql_db import assess_mssql_config
        return assess_mssql_config(raw, tenant_id, ref_id, framework_hint=framework)
