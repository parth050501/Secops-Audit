...
async def credits(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    # Credits removed — AI features are unmetered. Return an "unlimited" shape so
    # any older frontend reading this doesn't break.
    return {"total": None, "used": None, "remaining": None, "unlimited": True}
...
async def purchase_credits(body: AddCreditsReq, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    # Credits removed — purchasing is no longer applicable.
    raise HTTPException(status_code=410, detail="Credits have been removed; AI features are unmetered.")
