...
async def create_snapshot(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Capture the current compliance posture across all frameworks as a
    timestamped snapshot, building the historical record."""
    if user.role not in ("admin", "engineer"):
        raise HTTPException(status_code=403, detail="Not permitted")
    from app.services.snapshot_service import capture_snapshot
    created = await capture_snapshot(db, user.tenant_id, captured_by=user.name or "manual")
    return {
        "captured": len(created),
        "at": created[0].captured_at.isoformat() if created else None,
        "frameworks": [{"key": s.framework_key, "readiness_pct": s.readiness_pct} for s in created],
    }


@router.get("/compliance/history")
async def compliance_history(framework: str = None, days: int = 180,
                             user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Historical posture over time. Optionally filter to one framework. Returns
    snapshots ordered oldest-first, grouped by framework, for trend charts."""
    from datetime import datetime, timedelta
    from app.models.compliance_snapshot import ComplianceSnapshot
    since = datetime.utcnow() - timedelta(days=days)
    q = select(ComplianceSnapshot).where(
        ComplianceSnapshot.tenant_id == user.tenant_id,
        ComplianceSnapshot.captured_at >= since,
    )
    if framework:
        q = q.where(ComplianceSnapshot.framework_key == framework)
    q = q.order_by(ComplianceSnapshot.captured_at.asc())
    snaps = (await db.execute(q)).scalars().all()

    by_fw = {}
    for s in snaps:
        by_fw.setdefault(s.framework_key, {
            "key": s.framework_key, "name": s.framework_name, "points": [],
        })
        by_fw[s.framework_key]["points"].append({
            "at": s.captured_at.isoformat(), "readiness_pct": s.readiness_pct,
            "passing": s.passing, "failing": s.failing, "total": s.total_controls,
            "version": s.framework_version, "by": s.captured_by,
        })
    return {"frameworks": list(by_fw.values()), "snapshot_count": len(snaps)}



async def compliance_heatmap(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Cross-framework readiness at a glance — one cell per selected framework,
    each with its readiness %. Powers the compliance heatmap view."""
    from app.models.tenant import Tenant
    ...
    for fw_key in selected:
        ...
        readiness = round((passing / total) * 100) if total else 0
        cells.append({
            "key": fw_key, "name": fw.get("name", fw_key), "short": fw.get("short", fw_key),
            "color": fw.get("color"), "readiness_pct": readiness,
            "total_controls": total, "passing": passing, "failing": total - passing,
        })
    return {"frameworks": cells}


@router.get("/compliance/{fw_key}/detail")
async def compliance_detail(fw_key: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """Drill-down for one framework: controls grouped into families (by category),
    each family with its own readiness, and each control with pass/fail + findings.
    Powers the expandable Compliance Posture view."""
    from app.models.tenant import Tenant
    t = (await db.execute(select(Tenant).where(Tenant.id == user.tenant_id))).scalar_one_or_none()
    all_fw = await get_frameworks(db, user.tenant_id)
    fw = all_fw.get(fw_key)
    ...
    # group controls into families by category
    families = {}
    for c in controls:
    ...
    for fam, ctrls in sorted(families.items()):
        total = len(ctrls)
        passing = sum(1 for c in ctrls if c["status"] == "passing")
        family_out.append({
            "category": fam,
            "label": CATEGORY_LABELS.get(fam, fam.replace("_", " ").title()),
            "controls": ctrls,
            "total": total, "passing": passing, "failing": total - passing,
            "readiness_pct": round((passing / total) * 100) if total else 0,
        })
    # sort families worst-readiness first (surface problems, Wiz-style)
    family_out.sort(key=lambda f: f["readiness_pct"])
