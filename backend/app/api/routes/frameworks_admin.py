...
async def bulk_upload_controls(fw_id: int, file: UploadFile = File(...),
    ...
    # Optionally clear the framework's existing controls first (clean replace).
    removed = 0
    if replace_existing:
    ...
    if created or removed:
        await db.commit()
    return {"created": created, "removed": removed, "errors": errors[:50], "error_count": len(errors)}
