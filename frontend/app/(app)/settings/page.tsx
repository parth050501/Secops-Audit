...
import { getMyTenant, getFrameworks, updateTenant, getCredits, purchaseCredits, getAIUsage } from '@/lib/api';
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    getCredits().then(r => setCredits(r.data)).catch(()=>{});
  ...
  const buyCredits = async (amount: number) => {
    ...
    try {
      ...
      setCredits(r.data);
      setMsg(`${amount} credits added. (QC: simulated purchase — production uses Stripe)`);
    } finally { setSaving(false); }
  };
      ...
      <div className="p-6 max-w-3xl mx-auto">
        ...
        <div className="card p-6 mb-4">
          <div className="flex items-start justify-between mb-1">
            <div>
              <h2 className="text-sm font-semibold text-slate-700">AI Credits — Pay As You Go</h2>
              <p className="text-xs text-slate-400">AI is never used automatically. Each "Enhance with AI" click costs credits.</p>
            </div>
            {credits && (
              <div className="text-right">
                <div className="text-2xl font-bold text-violet-600">{credits.remaining}</div>
                <div className="text-xs text-slate-400">of {credits.total} credits</div>
              </div>
            )}
          </div>
          <div className="grid grid-cols-3 gap-3 mt-4 mb-4">
            {[{n:10,p:'$5'},{n:50,p:'$20'},{n:200,p:'$60'}].map(({n,p}) => (
              <button key={n} onClick={() => buyCredits(n)} disabled={saving}
                className="p-3 rounded-xl border-2 border-slate-100 hover:border-violet-300 hover:bg-violet-50 transition-all text-center">
                <div className="text-lg font-bold text-slate-800">{n}</div>
                <div className="text-xs text-slate-400">credits · {p}</div>
              </button>
            ))}
          </div>
          <div className="text-xs text-slate-400 mb-3">
            Costs: Enhance ticket = 1 credit · AI audit summary = 2 credits. Templates are always free.
          </div>
          {usage.length > 0 && (
            <div className="border-t border-slate-100 pt-3">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Recent AI usage</p>
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {usage.slice(0,8).map((u: any) => (
                  <div key={u.id} className="flex items-center gap-2 text-xs">
                    <span className="text-slate-400 w-32 flex-shrink-0">{new Date(u.timestamp).toLocaleString()}</span>
                    <span className="text-slate-600 flex-1">{u.operation.replace('_',' ')} by {u.user}</span>
                    <span className="font-mono text-violet-600">-{u.credits}</span>
                  </div>
                ))}
              </div>
            </div>
