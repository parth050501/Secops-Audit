...
import { platformLogin, platformOverview, platformTenants, platformUpdatePlan, platformUpdateStatus, platformDeleteTenant, platformImpersonate, platformOnboardTenant, platformListTeam, platformAddTeam, platformUpdateTeamRole, platformRemoveTeam, platformGetEmailSettings, platformUpdateEmailSettings, platformTestEmail, platformEditTenantInfo } from '@/lib/api';
import { Shield, Building2, DollarSign, Users, Plug, TrendingUp, LogIn, Trash2, AlertTriangle, Plus, X, UserCog, Mail, Settings as SettingsIcon, Edit2 } from 'lucide-react';
...
export default function PlatformConsole() {
  ...
  const [tab, setTab] = useState('tenants');           // tenants | email
  const [emailCfg, setEmailCfg] = useState<any>(null);
  const [emailSaving, setEmailSaving] = useState(false);
  const [testTo, setTestTo] = useState('');
  const [editTenant, setEditTenant] = useState<any>(null);   // tenant being edited
  ...
  const load = () => {
    ...
    loadTeam();
    platformGetEmailSettings().then(r => setEmailCfg(r.data)).catch(()=>{});
  ...
  const saveEmailCfg = async () => {
    setEmailSaving(true);
    try {
      const r = await platformUpdateEmailSettings(emailCfg);
      setEmailCfg(r.data); setMsg('Email settings saved.');
    } catch (e:any) { setMsg(e.response?.data?.detail || 'Could not save email settings'); }
    finally { setEmailSaving(false); }
  };
  const sendTest = async () => {
    if (!testTo.trim()) { setMsg('Enter a test recipient address'); return; }
    setMsg('Sending test email…');
    try { const r = await platformTestEmail(testTo); setMsg(r.data.message); }
    catch (e:any) { setMsg(e.response?.data?.detail || 'Test failed'); }
  };
  const saveTenantInfo = async () => {
    try {
      await platformEditTenantInfo(editTenant.id, {
        name: editTenant.name, industry: editTenant.industry,
        active_framework: editTenant.active_framework,
        custom_domain: editTenant.custom_domain, timezone: editTenant.timezone,
      });
      setMsg(`Tenant "${editTenant.name}" updated.`); setEditTenant(null); load();
    } catch (e:any) { setMsg(e.response?.data?.detail || 'Could not update tenant'); }
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-900">
        <div className="w-full max-w-sm">
          <div className="flex items-center justify-center gap-2.5 mb-6">
            ...
            <div>
              <div className="text-white font-bold">GRCBridge</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className="text-violet-300 text-xs">GRCBridge — all tenants</div>
            </div>
          </div>
        ...
        <div className="flex gap-1 mb-6 border-b border-slate-200">
          {[
            ...
            <button key={key} onClick={()=>setTab(key)}
              className={`flex items-center gap-2 px-4 py-2 text-sm border-b-2 -mb-px transition-colors ${tab===key?'border-violet-500 text-violet-700 font-medium':'border-transparent text-slate-500 hover:text-slate-700'}`}>
          ...
          <div className="max-w-2xl">
            <div className="card p-6">
              <div className="flex items-center gap-2 mb-1">
                ...
                <h2 className="font-semibold text-slate-800">Email / SMTP Configuration</h2>
              </div>
              <p className="text-xs text-slate-400 mb-4">Central email used to send reports and notifications to clients (e.g. via Amazon SES). Emails send from your configured address.</p>
              {emailCfg && (
                <div className="space-y-3">
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={!!emailCfg.email_enabled} onChange={e=>setEmailCfg({...emailCfg,email_enabled:e.target.checked})} />
                    Email sending enabled
                  </label>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs font-medium text-slate-600 mb-1 block">SMTP Host</label>
                      <input className="input" placeholder="email-smtp.us-east-2.amazonaws.com" value={emailCfg.smtp_host||''} onChange={e=>setEmailCfg({...emailCfg,smtp_host:e.target.value})} /></div>
                    <div><label className="text-xs font-medium text-slate-600 mb-1 block">Port</label>
                      <input type="number" className="input" placeholder="587" value={emailCfg.smtp_port||587} onChange={e=>setEmailCfg({...emailCfg,smtp_port:parseInt(e.target.value)||587})} /></div>
                  </div>
                  <div><label className="text-xs font-medium text-slate-600 mb-1 block">SMTP Username</label>
                    <input className="input" value={emailCfg.smtp_user||''} onChange={e=>setEmailCfg({...emailCfg,smtp_user:e.target.value})} /></div>
                  <div><label className="text-xs font-medium text-slate-600 mb-1 block">SMTP Password {emailCfg.smtp_password_set && <span className="text-emerald-600">(set — leave blank to keep)</span>}</label>
                    <input type="password" className="input" placeholder={emailCfg.smtp_password_set?'••••••••':'SES SMTP password'} value={emailCfg.smtp_password||''} onChange={e=>setEmailCfg({...emailCfg,smtp_password:e.target.value})} /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs font-medium text-slate-600 mb-1 block">From Address</label>
                      <input className="input" placeholder="alerts@codecoresystems.in" value={emailCfg.email_from||''} onChange={e=>setEmailCfg({...emailCfg,email_from:e.target.value})} /></div>
                    <div><label className="text-xs font-medium text-slate-600 mb-1 block">From Name</label>
                      <input className="input" placeholder="GRCBridge" value={emailCfg.email_from_name||''} onChange={e=>setEmailCfg({...emailCfg,email_from_name:e.target.value})} /></div>
                  </div>
                  <label className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={emailCfg.smtp_use_tls!==false} onChange={e=>setEmailCfg({...emailCfg,smtp_use_tls:e.target.checked})} />
                    Use STARTTLS (recommended)
                  </label>
                  <div className="flex justify-end pt-2">
                    <button onClick={saveEmailCfg} disabled={emailSaving} className="btn btn-primary text-sm">{emailSaving?'Saving…':'Save Settings'}</button>
                  </div>
                  <div className="border-t border-slate-100 pt-3 mt-2">
                    <label className="text-xs font-medium text-slate-600 mb-1 block">Send a test email</label>
                    <div className="flex gap-2">
                      <input className="input flex-1" placeholder="you@example.com" value={testTo} onChange={e=>setTestTo(e.target.value)} />
                      <button onClick={sendTest} className="btn text-sm">Send Test</button>
                    </div>
                    <p className="text-[11px] text-slate-400 mt-1">Save settings first. Requires a working SMTP setup (e.g. SES out of sandbox).</p>
                  </div>
                </div>
              )}
            </div>
                  ...
                  <tr key={t.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-1 justify-end">
                        <button onClick={()=>setEditTenant({...t})} title="Edit info & domain"
                          className="btn text-xs py-1 px-2 text-slate-600"><Edit2 className="w-3.5 h-3.5"/></button>
      ...
      {editTenant && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50" onClick={()=>setEditTenant(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md" onClick={e=>e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="font-bold text-slate-800">Edit Tenant</h2>
              <button onClick={()=>setEditTenant(null)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5"/></button>
            </div>
            <div className="p-5 space-y-3">
              <div><label className="text-xs font-medium text-slate-600 mb-1 block">Name</label>
                <input className="input" value={editTenant.name||''} onChange={e=>setEditTenant({...editTenant,name:e.target.value})} /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-medium text-slate-600 mb-1 block">Industry</label>
                  <input className="input" value={editTenant.industry||''} onChange={e=>setEditTenant({...editTenant,industry:e.target.value})} /></div>
                <div><label className="text-xs font-medium text-slate-600 mb-1 block">Timezone</label>
                  <input className="input" placeholder="UTC" value={editTenant.timezone||''} onChange={e=>setEditTenant({...editTenant,timezone:e.target.value})} /></div>
              </div>
              <div><label className="text-xs font-medium text-slate-600 mb-1 block">Custom Domain</label>
                <input className="input" placeholder="client.codecoresystems.in" value={editTenant.custom_domain||''} onChange={e=>setEditTenant({...editTenant,custom_domain:e.target.value})} />
                <p className="text-[11px] text-slate-400 mt-1">Stored now. Add the subdomain in GoDaddy DNS; live routing is wired in a later step.</p></div>
            </div>
            <div className="px-5 py-4 border-t border-slate-100 flex justify-end gap-2">
              <button onClick={()=>setEditTenant(null)} className="btn text-sm">Cancel</button>
              <button onClick={saveTenantInfo} className="btn btn-primary text-sm">Save</button>
            </div>
          </div>
        </div>
      )}
