...
export const getComplianceHeatmap = () => api.get('/api/governance/compliance/heatmap');
export const getComplianceDetail = (fwKey: string) => api.get(`/api/governance/compliance/${fwKey}/detail`);
export const captureSnapshot = () => api.post('/api/governance/compliance/snapshot');
export const getComplianceHistory = (framework?: string, days = 180) =>
  api.get('/api/governance/compliance/history', { params: { framework, days } });
...
export const getReport = (level: string) => api.get(`/api/reports/${level}`);
export const downloadReportPdf = async (level: string, tenantName = 'report') => {
  const r = await api.get(`/api/reports/${level}/pdf`, { responseType: 'blob' });
  const url = URL.createObjectURL(new Blob([r.data], { type: 'application/pdf' }));
  const a = document.createElement('a'); a.href = url; a.download = `${tenantName}_${level}_report.pdf`; a.click();
  URL.revokeObjectURL(url);
};

// ── Notification preferences ──
export const getMyNotifPrefs = () => api.get('/api/notifications/me');
export const setMyNotifPrefs = (data: any) => api.patch('/api/notifications/me', data);
export const getNotifDefaults = () => api.get('/api/notifications/defaults');
export const setNotifDefault = (role: string, data: any) => api.patch(`/api/notifications/defaults/${role}`, data);

// ── Report scheduling + history ──
export const getReportSchedule = () => api.get('/api/reports/schedule/config');
export const setReportSchedule = (data: any) => api.patch('/api/reports/schedule/config', data);
export const sendReportsNow = () => api.post('/api/reports/schedule/send-now');
export const getReportHistory = () => api.get('/api/reports/history/list');
export const saveReportToHistory = (level: string) => api.post(`/api/reports/history/save/${level}`);
export const downloadStoredReport = async (id: number, title = 'report') => {
  const r = await api.get(`/api/reports/history/${id}/pdf`, { responseType: 'blob' });
  const url = URL.createObjectURL(new Blob([r.data], { type: 'application/pdf' }));
